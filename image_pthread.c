...
void *threaded_conv(void *args){
  ...
  int start_row = input->rank * local_height;
  int end_row = (input->rank+1) * local_height;
  ...
  for (row=start_row; row<end_row; row++){
    for (pix=0; pix<input->srcImage->width; pix++){
      for (bit=0; bit<input->srcImage->bpp; bit++){
        input->destImage->data[Index(pix,row,input->srcImage->width,bit,input->srcImage->bpp)]=getPixelValue(input->srcImage,pix,row,bit,algorithms[input->type]);
      }
    }
  }
